...
import { Activity, Wand2, Clock, Settings, LogOut, ChevronDown, Sparkles } from "lucide-react";
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {[
              ...
              { to:"/pricing",   icon:<Sparkles className="w-4 h-4"/>, label:"Pricing" },
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <div className="relative">
                <button onClick={() => setUserMenu(!userMenu)}
                  ...
                  <div className="hidden sm:block text-left">
                    <div className="text-xs font-medium leading-none">{user.name?.split(" ")[0] || "Account"}</div>
                    <div className="text-[10px] font-mono uppercase tracking-wider text-[var(--raven-glow)] mt-1 leading-none">{user.tier || "free"}</div>
                  </div>
                ...
                {userMenu && (
                  <div className="absolute right-0 top-full mt-2 w-48 rounded-xl bg-[var(--surface)] border border-white/10 shadow-2xl overflow-hidden z-50">
                    ...
                    <Link to="/pricing" onClick={() => setUserMenu(false)}
                      className="flex items-center gap-2 px-4 py-3 text-sm text-[var(--muted)] hover:bg-white/5 hover:text-[var(--text)] transition-colors">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link to="/register" className="px-4 py-2 text-sm font-semibold bg-gradient-to-r from-[var(--raven)] to-[var(--raven-blue)] hover:brightness-110 shadow-[0_4px_16px_rgba(124,92,191,0.35)] text-white rounded-lg transition-all">Free Sign Up</Link>
              </div>
            )}
          </div>
        </div>
